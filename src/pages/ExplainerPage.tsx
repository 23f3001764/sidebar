...
export default function ExplainerPage() {
  ...
  useEffect(() => {
    ...
    const t = setInterval(() => setSlideIdx(p => (p + 1) % selected.content.length), 6000);
  ...
  useEffect(() => {
    ...
    return () => clearInterval(t);
  }, [carouselPaused, featuredCount, explainers.length]);
  ...
  return (
    <SteamiLayout>
      <motion.div className="mb-8" variants={fadeInUp} initial="hidden" animate="visible">
        <h1 className="steami-heading text-3xl md:text-4xl mb-3">🧠 Intelligence Explainers</h1>
            ...
            <div className="relative overflow-hidden" style={{ minHeight: 280 }}>
              <AnimatePresence mode="wait">
                <motion.div key={carouselIdx}
                  ...
                  exit={{ opacity: 0, x: -80 }} transition={{ duration: 0.45, ease: [0.4, 0, 0.2, 1] }}>
                  {(() => {
            ...
            <motion.div variants={modalVariants} initial="hidden" animate="visible" exit="exit"
              ...
              <div className="sticky top-0 z-10 px-7 py-4 flex items-center justify-between border-b border-foreground/5"
                ...
                <div className="flex items-center gap-2">
                  ...
                  <motion.button whileHover={{ scale: 1.08 }} whileTap={{ scale: 0.92 }}
                    onClick={() => setAutoPlay(!autoPlay)} className="steami-btn py-1.5 px-2.5 text-[9px]">
                    {autoPlay ? <Pause className="w-3 h-3" /> : <Play className="w-3 h-3" />}
                  </motion.button>
                  <motion.button whileHover={{ scale: 1.1 }} whileTap={{ scale: 0.9 }} onClick={closeModal}
                  ...
                  <div className="flex-1">
                    ...
                    <AnimatePresence mode="wait">
                      <motion.div key={slideIdx}
                        ...
                        <div className="text-sm font-light leading-relaxed text-muted-foreground mb-4 pl-5 border-l-2 border-steami-gold/50"
                    ...
                    <div className="mt-6 mb-4">
                      <KnowledgeGraph centerTopic={selected.title}
                        relatedTopics={selected.keyInsights.slice(0, 4)} field={selected.field} />
                    </div>
                    <div className="flex items-center justify-between mt-6">
                      <motion.button whileHover={{ scale: 1.05, x: -2 }} whileTap={{ scale: 0.95 }}
                        ...
                        disabled={slideIdx === 0} style={{ opacity: slideIdx === 0 ? 0.3 : 1 }}>
