/**
 * InsightPopup — redesigned to match ChatDashboard glass layout.
 *
 * Changes:
 *  • Layout: same max-w, same glass card style as ChatDashboard
 *  • Share button added (left of close ✕): shares to WhatsApp, Telegram,
 *    Reddit, LinkedIn, Instagram
 *  • SVG rendered via data-uri img with innerHTML fallback
...
// Social share icons as inline SVG strings (no external deps)
const SHARE_APPS = [
  {
    name: "WhatsApp",
    color: "#25D366",
  ...
  },
  {
    ...
    url: (u: string) =>
      // Instagram doesn't support direct share URLs — copy to clipboard as fallback
      `https://www.instagram.com/`,
  },
] as const;

interface Props {
  article: Article | { title: string; article_url?: string; url?: string; source?: string; published_at?: string; matched_domains?: string[]; };
  insight: AiInsight;
  onClose: () => void;
}
...
export default function InsightPopup({ article, insight, onClose }: Props) {
  ...
  const readUrl = insight.article_url
    || (article as any).article_url
    || (article as any).url
    || "";

  const shareTitle = `${article.title} — STEAMI AI Insight`;
  ...
  return (
    <div
      ...
      onClick={onClose}
    >
      {/* Glass card — full screen on mobile, constrained on desktop */}
      <div
        role="dialog"
        aria-modal="true"
        className="relative flex flex-col w-full sm:max-w-[520px] sm:rounded-2xl overflow-hidden shadow-2xl
      ...
      >
        ...
        <div className="flex items-center justify-between px-4 py-3 border-b border-white/[0.07] shrink-0">
          <div className="flex items-center gap-2">
            <span className="text-[9px] font-bold tracking-[0.2em] uppercase text-indigo-400">✦ AI Insight</span>
            {insight.domain && (
          ...
          <div className="flex items-center gap-1.5">
            ...
            <div className="relative">
              <button
                ...
                className="p-1.5 rounded-full bg-white/[0.07] hover:bg-white/15
              ...
              {shareOpen && (
                <div
                  className="absolute right-0 top-9 z-10 flex flex-col gap-1 p-2 rounded-xl shadow-2xl"
                  style={{
                ...
                >
                  {SHARE_APPS.map(app => (
                    <a
                      key={app.name}
                      href={app.url(readUrl, shareTitle)}
                      target="_blank"
                      rel="noopener noreferrer"
                      onClick={() => {
                        // Instagram fallback: copy link
                        if (app.name === "Instagram" && readUrl) {
                    ...
                    >
                      ...
                      {app.name === "Instagram" && (
                        <span className="text-white/25 text-[9px]">(copy)</span>
                      )}
                    </a>
                  ))}
                </div>
            ...
            <button onClick={onClose} aria-label="Close"
              className="p-1.5 rounded-full bg-white/[0.07] hover:bg-white/20
                text-white/50 hover:text-white transition-colors">
              <X size={13} />
            </button>
          </div>
        ...
        <div className="flex-1 overflow-y-auto" style={{ scrollbarWidth: "thin" }}>
          ...
          {hasSvg && (
            <div className="w-full flex items-center justify-center overflow-hidden"
              style={{ background: "rgba(99,102,241,0.04)", borderBottom: "1px solid rgba(255,255,255,0.06)", minHeight: 120 }}>
              {svgUri ? (
                <img src={svgUri} alt="AI visualisation" width={400} height={400}
                  className="max-w-full" style={{ maxHeight: 280, display: "block" }}
                  onError={e => {
                    const img = e.currentTarget as HTMLImageElement;
                    const wrap = img.parentElement!;
                    img.remove();
                    const div = document.createElement("div");
                    div.style.cssText = "max-width:100%;max-height:280px;overflow:hidden;";
                    div.innerHTML = svg;
                    wrap.appendChild(div);
                  }} />
              ) : (
                <div className="max-w-full overflow-hidden" style={{ maxHeight: 280 }}
          ...
          <div className="px-5 py-4">
            ...
            {/* Title */}
            <h3 className="text-white font-bold text-[15px] leading-snug mb-3">{article.title}</h3>

            {/* Summary */}
            <p className="text-white/70 text-[13px] leading-[1.7] mb-4">{insight.summary}</p>

            {/* Key points */}
            {Array.isArray(insight.key_points) && insight.key_points.length > 0 && (
              <div className="mb-4">
                <p className="text-white/25 text-[9px] font-bold uppercase tracking-[0.18em] mb-2">Key Points</p>
            ...
            {Array.isArray(insight.tags) && insight.tags.length > 0 && (
              <div className="flex flex-wrap items-center gap-1.5 mb-4">
                ...
                {insight.tags.map(tag => (
                  <span key={tag} className="px-2 py-0.5 rounded-full bg-white/[0.07] text-white/35 text-[10px]">{tag}</span>
                ))}
              </div>
            )}
            ...
            <div className="flex items-center justify-between pt-3 border-t border-white/[0.07]">
              <div className="flex items-center gap-1.5 text-white/20 text-[10px]">
                {(article as any).source && <span>{(article as any).source}</span>}
                {(article as any).published_at && (
                  <span>· {new Date((article as any).published_at).toLocaleDateString("en-GB", { day: "numeric", month: "short", year: "numeric" })}</span>
                )}
              </div>
              {readUrl && (
                <a href={readUrl} target="_blank" rel="noopener noreferrer"
                  className="flex items-center gap-1 text-indigo-400 hover:text-indigo-300 text-[11px] font-medium transition-colors">
