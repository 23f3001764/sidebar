/**
 * InsightPopup — updated
 *
 * Key change: calls setDeepLinkParam("insight", articleId) when the popup
 * mounts so the browser URL immediately becomes shareable:
 *   http://localhost:8080/?insight=<article_id>
 *
 * The Share button copies THAT URL (window.location.href) to clipboard
 * and to each social platform, so recipients can open the exact same popup.
 *
 * On close, ?insight= is removed from the URL.
 */
...
// Article prop is intentionally loose — accepts both Article and FeedArticle
...
export default function InsightPopup({ article, insight, onClose }: Props) {
  ...
  // ── Set ?insight= in URL when popup mounts so it's immediately shareable ──
  useEffect(() => {
    if (articleId) {
      setDeepLinkParam("insight", articleId);
    }
    return () => {
      // Remove only if it was set by THIS popup
      if (articleId) clearDeepLinkParam("insight");
    };
  ...
  // Share URL = current page URL (already has ?insight= set above)
  ...
  return (
    ...
    >
      ...
      <div
        role="dialog"
        aria-modal="true"
        className="relative flex flex-col w-full sm:max-w-[520px] sm:rounded-2xl overflow-hidden shadow-2xl h-full sm:h-auto sm:max-h-[90vh]"
        style={{
      ...
      >
        ...
        <div className="flex items-center justify-between px-4 py-3 border-b border-white/[0.07] shrink-0">
          ...
          <div className="flex items-center gap-1.5">
            ...
            <button
              ...
              className="flex items-center gap-1 p-1.5 rounded-full bg-white/[0.07] hover:bg-white/15
              ...
              {shareOpen && (
                ...
                >
                  {SHARE_APPS.map(app => (
                    <a
                      key={app.name}
                      href={app.url(shareUrl, shareTitle)}
                      target="_blank"
        ...
        <div className="flex-1 overflow-y-auto" style={{ scrollbarWidth: "thin" }}>
          ...
          <div className="px-5 py-4">
            ...
            {/* Summary */}
            <p className="text-white/70 text-[13px] leading-[1.7] mb-4">
              {insight.summary}
            </p>
